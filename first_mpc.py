...
#define (the certain) parameters of the system
A = np.array([
...
#Give the linear system to the model
x_next = A@_x + B@_u

#set up y and z for measurement and cost function
y_next = C@_x    #must return a casadi structure obtained via estimator's sy_template
                #that is if we want to use it as y_fun

#Setup using "right-hand-side" method
model.set_rhs('x', x_next)
model.set_rhs('y', y_next)



Z_next = y_to_z(y_next)

#set cost function to be used by controller later
model.set_expression(expr_name='cost', expr=sum1((yD - _y)**2))

#add z as an "auxilliary" expression to plot later
model.set_expression(expr_name='Z', expr=Z_next)
...
setup_mpc = {
    ...
    'n_horizon': 7, #this will lead to 7000 values as in Jake's code
    't_step': 0.01, #trying this value as neuron firing can be on the order of 10e-3 seconds
...
mpc.set_rterm(u=1e-5)          #see 'getting started: MPC" for l, m, and r terms
...
mpc.bounds['upper','_u','u'] = 4.228704824986444

#finalize our controller
mpc.setup()

#Assuming all states can be directly measured - which we have via our equation right?
estimator = do_mpc.estimator.StateFeedback(model) 

#Creating a simultor to run MPC in a closed loop
simulator = do_mpc.simulator.Simulator(model)
simulator.set_param(t_step = 0.01) #same time step as above
simulator.setup()

#Now to run the closed loop control
#setting an initial state as in Jake's
x0 = np.array([[1.43686192], [-1.45507814], [-1.70993785], [-1.59286133], [1.7252861]])     
...
# #save_results([mpc, simulator]) - commented out so we don't make duplicates

# results = load_results('./results/results.pkl')

# ysults = results['mpc']['_x', 'y']
# print(ysults)
